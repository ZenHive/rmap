...
#[test]
fn render_rejects_unclosed_milestones_marker() {
    let tasks = validate_tasks_str("roadmap/tasks.toml", TASKS).expect("valid tasks");
    let roadmap = "<!-- MILESTONES:BEGIN -->\nstale generated content\n";

    let err = render_roadmap_str_with_today(roadmap, &tasks, "2026-05-11")
        .expect_err("unclosed MILESTONES marker is rejected");

    assert!(err.to_string().contains("missing <!-- MILESTONES:END -->"));
}
