...
pub struct NewTaskFields<'a> {
    ...
    pub phase: u32,
    pub bundle: &'a str,
    /// Repository where this task's own work lands. `None` defaults to the
    /// roadmap's top-level `project` and skips serialization.
...
pub fn add_task_str(
    ...
    if let Some(target_repo) = fields.target_repo {
        table["target_repo"] = Item::Value(Value::from(target_repo));
    }
