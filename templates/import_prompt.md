...
Status glyphs: ⬜ → pending, 🔄 → in_progress, ✅ → done, ⛔ → blocked, 🔁 → superseded
...
- Preserve task ids from the source document where they exist; auto-allocate (omit `id`)
  for rows where ids were absent — `rmap new --from-stdin` will assign them.
