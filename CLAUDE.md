...
- `render_html.rs` — `rmap render --html` single-project view via the one minijinja template (`templates/roadmap.html.j2`, `include_str!`'d). Builds DAG via the longest-path layering in `topo.rs`.
- `topo.rs` — pure longest-path layering `compute_layers(tasks) -> {id → depth}` over the in-repo `depends_on` graph (extracted from `render_html`). Shared by `render_html.rs` (DAG vertical slotting) and `export.rs` (the computed `dep_layer` field).
...
- **`--json` outputs of `show` / `list` / `next` / `next-bundle` / `ready` / `bundles` / `schema` / `diff` / `doctor` are additive-only.** Add fields freely; rename/remove → `schema_version` bump.
