...
- **Timestamp fields are ISO-8601 dates.** `created_at`, `started_at`, `done_at`, and `scored_at` are all `Option<String>` validated against the `YYYY-MM-DD` shape (10 chars: 4 digits, dash, 2 digits, dash, 2 digits). Don't promote them to a typed `Date` — the value lives next to user-edited TOML where exact string preservation matters.
