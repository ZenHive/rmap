...
**Status.** Phases 1–5, 8, 9, 10, 11a, and 12a shipped, plus the `mark`, `depend`, delegate per-agent footer, and `diff --verbose` polish from 11b, and the two `doctor` lints (degenerate bundles, missing acceptance_criteria) from 12d: `validate`, `render`, `data.json` export, `status`/`mark`/`depend` mutators (status: single + bulk), `next` selector, `show`, `list`, `schema`, `diff` (with `--verbose` value-level before/after), `delegate` (with per-agent environment-notes footer), `stale`, `doctor` (validation + stale + score-decay + degenerate-bundle + missing-AC + drift), the `--check-render` pre-commit contract, schema completeness (`[focus]`, task timestamps, `blocked_reason`, cycle detection), health surfaces (score-decay `?` suffix, stale-task surface, composite doctor report), and an end-to-end dogfood of `portfolio_dashboard` (20 tasks / 7 phases authored from `dashboard_roadmap.md`; all read + write + diff + delegate commands round-trip on a real consumer repo). A follow-up audit pass (Phase 12d) cross-checked the dogfood output against `~/.claude/includes/task-prioritization.md`, `task-writing.md`, and three reference ROADMAPs (`ccxt_extract`, `onchain`, `rexex`) and surfaced 4 tool gaps — the two `doctor` lints are now shipped; the auto-rendered Current Focus block and the optional `module` field on Task remain in Phase 12d. Phases 6–7 and the remainder of 11b (mermaid render, `new`/`new --from-stdin`) are planned — see *Implementation phases* below. The next active slice is **Phase 12b: `SKILLS.md` paired with 12d.1 (auto-rendered FOCUS block)** so cloud agents can drive rmap without rediscovering the surface each session. The contract is **write-once, read by both agents and humans**: the same `tasks.toml` feeds an agent-queryable JSON view and a human-readable Markdown view.
...
| 12d | **Audit follow-ups** | 🔄 | Two `doctor` lints shipped (`degenerate_bundle` ✅, `missing_acceptance_criteria` ✅). Remaining: auto-FOCUS render block (12d.1), optional `module` field on Task (12d.3). |
...
1. **Auto-rendered Current Focus block.** All three reference roadmaps carry a `## Current Focus` section with multi-sentence narrative: *Last shipped: <task> on <date>*, *Up next: Task N (Eff:X)*, in-progress count, focus-phase callout. rmap has every input (`[focus].phase`, `done_at`, `next::next_task` selector, in-progress count) but the renderer emits none of it. The `[focus]` field today only influences `next` selection — under-leveraged. Shape: new marker pair `<!-- FOCUS:BEGIN -->` / `<!-- FOCUS:END -->` mirroring the `TASKS:` semantics; renderer replaces the body with `**Focus phase:** N — <name> (M of K done · J in progress)` + `**Last shipped:** Task A, Task B on YYYY-MM-DD` (from `done_at` within 7d; "no recent shipments" when empty) + `**Up next:** Task C — <title> [D:.../B:.../U:... → Eff:...]`. Same byte-preservation rule as `TASKS:`. `validate --check-render` extends to cover focus-block drift. Zero-config default: if the markers aren't present in `ROADMAP.md`, no focus block is rendered. Touches `src/render.rs`, `tests/golden/focus_block/`, `CLAUDE.md` (Load-bearing invariants — new marker pair). [D:4/B:8/U:8 → Eff:2.0] 🚀
...
3. **Optional `module` field on `Task`.** `onchain/ROADMAP.md` Phase 7/8/Code-Health tables carry a `Module` column (`Onchain.ERC20`, `Onchain.Subscription`, `Onchain.RPC.Helpers`) tying each task to its primary module / file / path — useful for fan-out across code-heavy repos. rmap's fixed render template has no slot. Add `module: Option<String>` on `schema::Task`; render as a small inline tag between the bundle gift-tag and the title: `🎁 **bundle** · *<module>* · <title> [D/B/U]`. Three-place edit per the existing invariant (line 149): `schema::Task` + `diff::diff_fields!` invocation + `export::ExportedTask`. Decide whether `module` belongs on `TASK_VERBOSE_WHITELIST` (probably yes — small string, agents reading `diff --verbose` benefit). Touches `src/schema.rs`, `src/diff.rs`, `src/export.rs`, `src/render.rs`, `CLAUDE.md`, `tests/golden/module_field/`. [D:3/B:5/U:5 → Eff:1.67] 🚀
...
Sequence by Eff: 12d.2 (2.25 🎯) ✅ → 12d.1 (2.0 🚀, biggest convention payoff) → 12d.4 (2.0 🚀) ✅ → 12d.3 (1.67 🚀). 12d.2 + 12d.4 shipped together (bundle 1) — both touch `src/doctor.rs` + `DoctorFinding` + `tests/cli.rs` shape. 12d.1 + 12d.3 + 12b (`SKILLS.md`) are bundle 2 — render-surface + doc-surface, SKILLS.md naturally documents both new render slots.
